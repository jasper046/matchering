...
from .stages import main, main_with_preset, analyze_reference
...
def process_with_preset(
    ...
    # Validation of the most important conditions
    if (
        not (target_sample_rate == config.internal_sample_rate)
    ...
    # Process
    result, result_no_limiter, result_no_limiter_normalized = main_with_preset(
        target,
        preset,
        config,
        need_default=any(rr.use_limiter for rr in results),
...
def analyze_reference_track(reference: str, preset_path: str, config: Config = Config()):
    debug(
        "Please give us a star to help the project: https://github.com/sergree/matchering"
    )
    debug_line()
    ...
    # Validation of the most important conditions
    if (
        not (reference_sample_rate == config.internal_sample_rate)
        or not (channel_count(reference) == 2)
        or not (size(reference) > config.fft_size)
    ):
        raise ModuleError(Code.ERROR_VALIDATION)
